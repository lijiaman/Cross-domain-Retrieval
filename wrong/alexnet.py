...
def AlexNet(rgb):
    ...
    network = tl.layers.DenseLayer(network, n_units=4096, act = tf.tanh, name = 'fc6')
    network = tl.layers.DropoutLayer(network, keep=0.5, name='drop1')
    network = tl.layers.DenseLayer(network, n_units=4096, act = tf.tanh, name = 'fc7')
    network = tl.layers.DropoutLayer(network, keep=0.5, name='drop2')
    network = tl.layers.DenseLayer(network, n_units=50, act = tf.identity, name = 'fc8')

    print("build model finished: %fs" % (time.time() - start_time))
    return network

model_file_name = "adam_model_deepfashion.ckpt"
resume = True
