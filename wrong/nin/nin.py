...
x_nopair = tf.placeholder("float", [None, 227, 227, 3])
#y_nopair_ = tf.placeholder(tf.int32, shape=[batch_size,])


fc1_street, street_nin = NIN(x_street,'street')
fc1_shop, shop_nin = NIN(x_shop,'shop')
fc1_nopair, nopair_nin = NIN(x_nopair,'shop',reuse=True)
...
for epoch in range(n_epoch):
    ...
    for iter in xrange(iter_per_epoch):
    	street_batch, shop_batch, nopair_batch, y_street_batch, y_shop_batch, y_nopair_batch = input.load_batchsize_images(batch_size)
        #print "street_batch.shape:"
        #print street_batch.shape
        #print "iter:"
        #print iter
        feed_dict = {x_street: street_batch, x_shop: shop_batch, x_nopair: nopair_batch}
        feed_dict.update( dict(street_nin.all_drop.items()+shop_nin.all_drop.items()) )        # enable all dropout/dropconnect/denoising layers
        err, d_pair, d_nopair, t_lr, _, train_summary = sess.run([triplet_loss, dist_pair, dist_nopair, triplet_lr, train_op, merged], feed_dict=feed_dict)
        iter_show += 1
    	...
    	val_street_batch, val_shop_batch, val_nopair_batch, val_y_street_batch, val_y_shop_batch, val_y_nopair_batch = input.load_val_images(batch_size)
        ...
        feed_dict = {x_street: val_street_batch, x_shop: val_shop_batch, x_nopair: val_nopair_batch}
        feed_dict.update(dp_dict)        # enable all dropout/dropconnect/denoising layers
        err, d_pair, d_nopair, val_summary = sess.run([triplet_loss, dist_pair, dist_nopair, merged], feed_dict=feed_dict)
    ...
    if epoch + 1 == 1 or (epoch + 1) % print_freq == 0:
        ...
        for iter_test in xrange(test_iters):
            ...
            dp_dict = tl.utils.dict_to_one( dict(street_nin.all_drop.items()+shop_nin.all_drop.items()) )    # disable all dropout/dropconnect/denoising layers
            feed_dict = {x_street: street_test_batch, x_shop: shop_test_batch, x_nopair: nopair_test_batch}
            feed_dict.update(dp_dict)
            err, d_pair, d_nopair = sess.run([triplet_loss, dist_pair, dist_nopair], feed_dict=feed_dict)
...
#
        ...
        with open('toy50_test_result.txt', 'a') as w_f:
...
#        print("   test nopair loss:{0}, acc:{1}, lr:{2}".format(test_nopair_loss, test_nopair_acc/test_iters, nopair_test_lr))
    if (epoch + 1) % 1 == 0:
        ...
        saver = tf.train.Saver()
        file_path = "/ais/gobi4/fashion/data/"
        file_name = "toy_retrieval_nin_"+str(epoch)+".ckpt"
