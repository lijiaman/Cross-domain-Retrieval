...
triplet_loss_vec = tf.maximum(dist_vec, zero)
...
for epoch in range(n_epoch):
    ...
    iter_per_epoch = 3068#98163/32=3067.6
    #iter_per_epoch = 1
    for iter in xrange(iter_per_epoch):
    	#street_batch, shop_batch, nopair_batch, _, _, _ = random_input.load_batchsize_images(batch_size)
    	street_batch, shop_batch, nopair_batch, _, _, _ = random_input.load_batchsize_images(batch_size, 'train')
    ...
    val_triplet_loss, val_dpair, val_nopair, batch_val_size = 0, 0, 0, 32
    val_iters = 1518
      
    for iter_val in xrange(val_iters):
        street_val_batch, shop_val_batch, nopair_val_batch, y_street_val_batch, y_shop_val_batch, y_nopair_val_batch = random_input.load_batchsize_images(batch_val_size,'val')
...
#
    ...
    if epoch + 1 == 1 or (epoch + 1) % print_freq == 0:
        ...
        test_iters = 1481#47384/32=1480.7
        #test_iters = 1
        ...
        for iter_test in xrange(test_iters):
            street_test_batch, shop_test_batch, nopair_test_batch, y_street_test_batch, y_shop_test_batch, y_nopair_test_batch = random_input.load_batchsize_images(batch_test_size,'test')
